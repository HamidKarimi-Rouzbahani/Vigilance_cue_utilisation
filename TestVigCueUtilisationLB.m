%% Changed by Rebecca 29/6/2022
%% Changed Blocks_per_condition = 15 to Blocks_per_condition = 5 due to RAM issue - line 51
%% Now just run this script 3 times 
%% LB = Less Blocks

function TestVigCueUtilisationLB(Subj,percentage_target_cond,chunk)
...
% modified by Hamid Karimi-Rouzbahani on 1/July/2022: took the percentages 
% out of the function for manual counterbalancing; also split each target
% frequency into chunks which needs to be imported to the function
% also changed rng ('default') to rng(3) to avoid freezing as a result of
% random generation
% also increased the number of subjects from 20 to 100 to allow for more
...
Blocks_per_condition= 5; % Number of blocks per target frequency condition - was 15 
...
try
    ...
    for Block_Num=1:Num_of_conditions*Blocks_per_condition
        ...
        save(['Subj_',num2str(Subj),'_Blk_',num2str(Block_Num),'_Chunk_',num2str(chunk),'_',Condition_string,...
            '_test_CueUtilisation.mat'],'manually_deflected_top','top_events','top_events2','top_targets','top_targets2',...
